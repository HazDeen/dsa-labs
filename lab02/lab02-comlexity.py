...
Запуск: python lab02-complexity.py --variant N
"""
...
def factorial(n: int) -> int:
    """Факториал n >= 0 рекурсивно. Ожидаемая сложность: O(n) время, O(n) память."""
    # Граничный случай: факториал отрицательных чисел не определен
    if n < 0:
        raise ValueError("Факториал определен только для неотрицательных чисел")
    
    # Базовое условие (base case): 0! = 1 и 1! = 1.
    # Здесь рекурсивный спуск останавливается.
    if n in (0, 1):
        return 1
...
def fib_naive(n: int) -> int:
    ...
    Ожидаемая сложность: O(2^n) время (экспоненциальная), O(n) память на стек.
    """
    # Фиксируем каждый факт входа в функцию
    CALLS["fib_naive"] += 1
    ...
    # Рекурсивный переход: ветвление на две подзадачи
...
def fib_memo(n: int, memo: dict[int, int] | None = None) -> int:
    ...
    Ожидаемая сложность: O(n) время (линейная), O(n) память (стек + словарь).
    """
    # Фиксируем вызов (включая обращения к кешированным веткам)
    CALLS["fib_memo"] += 1

    # Инициализация словаря memo:
    # Задаем базовые случаи F(0) = 0 и F(1) = 1 сразу в кеше
    if memo is None:
        memo = {0: 0, 1: 1}

    # Если значение уже вычислено ранее — мгновенный возврат за O(1)
    if n in memo:
        return memo[n]

    # Иначе вычисляем рекурсивно и сохраняем в memo перед возвратом
...
def hanoi(n: int, src: str = "A", dst: str = "C", aux: str = "B") -> int:
    ...
    """
    # Базовое условие: если дисков нет (n == 0), перемещений не требуется
    if n <= 0:
        return 0

    # Алгоритм «Разделяй и властвуй»:
    # 1. Переместить (n - 1) дисков со стержня src на aux (используя dst как буфер)
    moves = hanoi(n - 1, src=src, dst=aux, aux=dst)

    # 2. Переместить 1 оставшийся самый большой диск с src на dst
    moves += 1

    # 3. Переместить (n - 1) дисков с aux на dst (используя src как буфер)
    moves += hanoi(n - 1, src=aux, dst=dst, aux=src)
...
class DynamicArray:
    ...
    def append(self, value) -> None:
        ...
        Худший случай отдельной вставки (с расширением): O(n).
        """
