...
def array_sum(a: list[int]) -> int:
    ...
    Объяснение: обычный цикл for проходит по всем n элементам по очереди
    и прибавляет каждый к переменной total."""
    total = 0
    for x in a:
        total += x
    return total
...
def array_max(a: list[int]) -> int:
    ...
    Объяснение: берем первый элемент за максимум и в цикле сравниваем его
...
def count_equal_pairs(a: list[int]) -> int:
    ...
    Объяснение: два вложенных цикла по индексам. Первый берет элемент i,
    второй идет по всем элементам j правее него и ищет совпадения.
    """
    count = 0
    n = len(a)
    for i in range(n):
...
def binary_pow(x: int, n: int, mod: int | None = None) -> int:
    ...
    Объяснение: пока показатель степени n > 0, смотрим на остаток от деления
    на 2. Если нечетное — умножаем результат на основание, затем основание
    возводим в квадрат, а степень делим нацело на 2. Если передан mod,
    сразу берем остаток, чтобы числа не разрастались.
    """
    res = 1
    if mod is None:
        base = x
        ...
        base = x % mod
    while n > 0:
        # Если бит нечетный — умножаем накопленный результат
        if n % 2 == 1:
            res *= base
            if mod is not None:
                res %= mod
        # В любом случае возводим базу в квадрат и делим степень пополам
        base *= base
        if mod is not None:
            base %= mod
        n //= 2
